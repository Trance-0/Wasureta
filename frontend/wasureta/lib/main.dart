...
class _MyHomePageState extends State<MyHomePage> {
  ...
  Future<ListView> getTestCards(String? tag) async {
    Uri uri = Uri.parse("${settings.BASE_URL}/jisho/getJishoList");
    if (tag != null) {
      uri = Uri.parse("${settings.BASE_URL}/jisho/getJishoList?tag=$tag");
    }
    ...
    if (response.statusCode == 200) {
      ...
      List<Jisho> posts =
          List<Jisho>.from(l.map((model) => Jisho.fromJson(model)));
      List<Widget> cards =
          List<Widget>.from(posts.map((e) => TestCard(jisho: e, lock: false)));
